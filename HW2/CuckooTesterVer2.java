...
public class CuckooHashTesterVer2 {
    public static void performCuckooTest(){
        long start;
        long totalTime;
        Random ran = new Random();//random number generator
        final int numHashFunction = 1;//number of hash function
        ...
        System.out.printf("%4s %20s %20s%20s %20s %20s%20s %20s %20s\n","Case", "Test Size", "Total Time","Case", "Test Size", "Total Time","Case", "Test Size", "Total Time");
        ...
        while(indexTest < testSize.length) {
            CuckooHashTable<Integer> cuckoo = new CuckooHashTable<Integer>(new IntegerHashFamily(numHashFunction),testSize[indexTest]*2);//we used a lot of space
